...
class GLDbMysql extends GLibrary
{
    ...
    public function addField($fields = [])
    {
        $this->sql =
            'SELECT ' .
            implode(',', $fields) .
            ltrim(implode(',', explode('SELECT', $this->sql, 2)), ',');
        return $this;
    }
    ...
    public function where_equal_query(
        $field,
        $query,
        $values,
        $type = '',
    ...
    {
        if (strlen($this->where) > 0) {
            $this->where .= $type . ' EXISTS (' . $query . ' ) ';
        } else {
            $this->where = 'WHERE EXISTS (' . $query . ' ) ';
        }
        $this->addParam($params);
        return $this;
    }
    ...
    public function where_filter($filters = [])
    {
        $i = 0;
                ...
                $this->where =
                    (strlen($this->where < 1)
                        ...
                        : (isset($filter['join'])
                            ...
                            : ' AND ')) .
                    ' ' .
                    (isset($filter['table']) ? $filter['table'] . '.' : '') .
                    $filter['field'] .
                    ' ' .
                    (isset($filter['sign']) ? $filter['sign'] : '=') .
                    ' ? ';
            } else {
                (isset($filter['join']) ? $filter['join'] : ' AND ') .
                    ' ' .
                    (isset($filter['table']) ? $filter['table'] . '.' : '') .
                    $filter['field'] .
                    ' ' .
    ...
    {
        $this->where .=
            strlen($this->where) < 1 ? 'WHERE ' . $condition : $condition;
        $this->addParam($values);
        return $this;
    }
    ...
    public function execute($option = 3)
    {
        ...
        if ($res) {
            count($this->parameters) > 0 &&
                $res->bind_param($this->string, ...$this->parameters);
            $res->execute();
            $this->resetSql();
            if (!$res->execute()) {
                return [
    ...
    public function insert($table, $data)
    {
        ...
        for ($i = 0; $i < sizeof($fields); $i++) {
            ...
            array_push(
                $values,
                $this->dbconn->real_escape_string($data[$fields[$i]])
            );
        }
        $sql =
            " INSERT INTO $table (" .
            implode(',', $fields) .
            ")  VALUES ( $plc_holder )";
        $prep_insert = $this->dbconn->prepare($sql);
        echo mysqli_error($this->dbconn);
        // echo mysqli_error($this->dbconn);
        if ($prep_insert) {
            ...
            if (!$prep_insert) {
                ...
                return $this->dbconn->insert_id != 0
                    ? $this->dbconn->insert_id
                    : true;
            }
        } else {
            return false;
    ...
    public function delete($table, $condition)
    {
        ...
        for ($i = 0; $i < count($clause); $i++) {
            $where = $i == 0 ? 'WHERE ' : '';
            $where .= $clause[$i];
            if (is_numeric($condition[$clause[$i]])) {
                $string .= 'i';
            } else {
                $string .= 's';
            }
            array_push($values, $condition[$clause[$i]]);
            if ($i == sizeof($clause) - 1) {
                $where .= '= ? ';
            } else {
                $where .= '= ? AND ';
            }
