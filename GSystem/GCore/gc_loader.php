<?php 
  ...
  class GCLoader{
	  ...
	  private function extractUrl($type,$path){
			...
			}else { $classname = $file = $path;}
			switch ($type) {
				case 'library':
					$url =  BASE_PATH.'/GSystem/GLibraries/'.$route.'gl_'.$file.'.php';
					$name = "library_".str_replace('/','_',$route).$file;
					break;
				case 'model':
					$url =  BASE_PATH.'/src/GModels/'.$route.'gm_'.$file.'.php';
					$name = "model_".str_replace('/','_',$route).$file;
					break;
				case 'controller':
					$url =  BASE_PATH.'/src/GControllers/'.$route.'gc_'.$file.'.php';
					$name = "controller_".str_replace('/','_',$route).$file;
					break;
				case 'route':
					$url =  BASE_PATH.'/src/GRoutes/'.$route.'gr_'.$file.'.php';
					$name = "route_".str_replace('/','_',$route).$file;
					break;   
				case 'helper':
					$url =  BASE_PATH.'/src/GHelpers/'.$route.'gh_'.$file.'.php';
					$name = "route_".str_replace('/','_',$route).$file;
					break;   
				default:
					# code...
			 ...
			 if(file_exists($url)){
				 ...
				 throw new Exception("Error: Loading $type  $path. File does not exist");
			 }
    }
		...
		public function library($name,$options = ['use_db' => false]){   
			try {
				...
				if(class_exists($className)){
					...
					LOAD_DATABASE && $options['use_db'] && $this->class->$name->db = $this->db;
					return $this->class->$name;
				}else{
					throw new Exception("Error: In-built libraries matching \"$name\". Class does not exist");
				}
			} catch (\Throwable $th) {
	  ...
	  public function model($name){
			try{
				...
				if(class_exists($className)){
					...
					throw new Exception("Error loading model $name. Class does not exist");
				}
			}catch (\Throwable $th) {				
				throw new GCLoaderError($th);
			}
	  ...
	  public function controller($name){
			try {
				...
				if(class_exists($className)){
					...
					return $this->class->$name  = $this->load_library_on_class(new $className());
				}else{
					throw new Exception("Error loading controller $name. Class does not exist");
				}
	  ...
	  public function route($route,$method){
			try {
				...
				if(method_exists($class, $method)){
					...
					throw new GCRouteError(new Exception("Error: Attempt to access route $route->$method. Method $method does not exist on class ".get_class($class)."."));
				}
			} catch (\Throwable $th) {
				new GCHandler($th);
				if($th instanceof  GCRouteError){
					$this->libraries['request']->emit(['status'=>false,'message'=>'The requested resource does not exist','code'=>404]);
				}
				$this->libraries['request']->emit(['status'=>false,'message'=>'unlucky','code'=>500]);
			}
    }
		...
		public function helper($path,$function,$params = []){
			try{
				...
				if(class_exists($className)){
					$name = $helper['name'];
					$this->class->$name  = $this->load_library_on_class(new $className());
					if(method_exists($this->class->$name,$function)){
						return $this->class->$name->$function($params);
					}else{
						throw new Exception("Error loading helper $name. Method does not exist");
					}
				}else{ throw new Exception("Error loading helper $path. Class does not exist");}
			} catch (\Throwable $th) {
    ...
    public function public($file){
      $this->libraries['request']->emitfile($file,$this->libraries['file']->getFileMimeType($file));
    }
