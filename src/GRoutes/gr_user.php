	...
	class GRUser Extends GRoute{
      ...
      // echo $this->library_encrypt->generate_api_private_key();
		...
		private function register(){
			$postFields = $this->request->post(array('username','telephone','email','password'));
		...
		public function login(){
			$postFields = $this->request->post(array('loginid','password'));
			$this->load->controller('user');
		...
		public function balance(){
			...
			$this->request->emit(['status'=>true,'balance'=>$balance,'message'=>"Your balance is $balance"]);
		}
