...
class GCTrader extends GController
{
  ...
  public function profile()
  {
    ...
    $trader = $this->model_trader->getTraderByUserId($user['userid']);
    $data['firstname'] = $trader['firstname'];
    $data['lastname']  = $trader['lastname'];
    $data['othername'] = $trader['othername'];
    $data['telephone'] = $trader['telephone'];
    $data['country'] = $trader['country'];
    $data['countryid'] = $trader['countryid'];
    $data['city'] = $trader['city'];
    $data['cityid']  = $trader['cityid'];
    $this->request->emit([
      'status' => true,
  ...
  public function getTraderCart($tradeId)
  {
    ...
    $userExists = $this->model_trader->getMTraderByTradeId($tradeId);
    if (!$userExists['status']) return ["error" => true, "message" => "Invalid User"];
    $cartData = $this->model_trader->getTraderCart($userExists['data']['mtraderid']);
    $cart = [];
    $cartData = $cartData ? $cartData : [];
    foreach ($cartData as $cartItem) {
      $cartItem['product'] =  $this->model_products->getProduct($cartItem['product_id']);
      $cart[] = $cartItem;
    }
